...
To be able to compare the two annotation files with intersectBed, we should uniform the file formats. To do that, make sure that the same columns correspond to start, end, strand and contig_id. The direct outputs of Phanotate and prodigal-gv do not match directly. Do that using the command below (alternatively, do the task in your own way).
