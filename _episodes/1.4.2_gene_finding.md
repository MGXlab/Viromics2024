...
>
> Do you think all genes predicted by prodigal-gv are real biological structures?
>
...
>
> BED is an alternative format to GFF. Describe the BED format.
> 
> {: .source}
{: .discussion}

You may have noticed that the direct outputs of Phanotate and prodigal-gv do not match directly. To be able to compare the two annotation files with intersectBed, we should uniform the file formats. To do that, make sure that the same columns correspond to start, end, strand and contig_id. Do that using the command below adapt the format of the prodigal-gv file (alternatively, do the task in your own way).
...
#Coordinates intersection to get unique elements of file *a* (adapt the script to save them to a file or just get the number of elements as above)
