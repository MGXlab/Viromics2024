...
Phanotate has a different internal approach than CheckV's [prodigal-gv](https://github.com/apcamargo/prodigal-gv). While Phanotate is more broad and predicts all possible ORFs using a graph approach, prodigal-gv predicts genes based on their properties. So, we expect to see differences. It will be interesting to compare the outputs of the two tools. For that let's compare the coordinates with [BEDtools intersectBed](https://bedtools.readthedocs.io/en/latest/content/tools/intersect.html).  
