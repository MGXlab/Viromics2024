...
Many phages naturally have [low-complexity regions](https://en.wikipedia.org/wiki/Low_complexity_regions_in_proteins) in their genomes (e.g. ACACACACACAC). Nanopore sequencing errors are biased towards low-complexity regions, either by falsely generating them, or by exacerbating them. This can create artifacts in the assembled viral genome sequences. High-quality reads can significantly improve assemblies, particularly for error-prone long reads. 
...
## GC Content
GC Content is another interesting metric to evaluate your sequences. There's no "wrong" GC content, rather this measures tells us about how much nucleic acid diversity our metagenomes/viromes might have. 

GC content is used in phylogeny as different taxonomic branches have differing GC content

Varying GC content within a sequence is often used to identify self/non-self regions. For example, a horizontally transferred gene, or an inserted mobile genetic element might have much higher or lower GC content to the neighbouring sequence space. 

High GC content means that the nucleic acids have a higher melting temperature. [A recent study showed that higher GC content is positively correlated with high temperature adaptation in prokaryotes](https://bmcgenomics.biomedcentral.com/articles/10.1186/s12864-022-08353-7#:~:text=As%20guanine%20(G)%20strictly%20pairs,to%20measure%20genomic%20nucleotide%20composition.)
