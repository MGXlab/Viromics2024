...
<p align="center">
    <a href="{{ site.carpentries_site }}"><img src="{{ relative_root_path }}/assets/img/logo_day1.png" alt="Viromics workflow" width="1200" /></a>
</p>
...
>
>for fn in $datadir/barcode6*.fastq.gz
>do
...
> 14. Describe the difference in GC content between our viromes and [E. coli phage T4](https://trace.ncbi.nlm.nih.gov/Traces/?view=run_browser&acc=SRR29341083&display=metadata) `./data/sequences/T4_SRR29341083.fastq`).  
> 15. What interpretations can you make about the samples based on these plots?  
>{: .source}
{: .challenge}

To make these GC_content plots, you will probably need the following python libraries:
- Bio.SeqUtils
- Bio.SeqIO
- pandas
- numpy
...
> pip install biopython, pandas, numpy, gzip, sys, matplotlib
>```
>
>```python
>#! usr/bin/python3
...
>gc_62={}
>with gzip.open(f"{fastq_dir}/barcode62.fastq.gz", 'rt') as f:
...
>
>gc_63={}
>with gzip.open(f"{fastq_dir}/barcode63.fastq.gz", 'rt') as f:
>    for record in SeqIO.parse(f, "fastq"):
>        gc_63[record.id]=gc_fraction(record.seq)*100
>print("gc_63... done")
>
>gc_64={}
>with gzip.open(f"{fastq_dir}/barcode64.fastq.gz", 'rt') as f:
>    for record in SeqIO.parse(f, "fastq"):
...
> python3 ./python_scripts/qc/gc_content.py ./data/sequences ./data/sequences/T4_SRR29341083.fastq.gz
>
> deactivate 
>```
>{: .source}
