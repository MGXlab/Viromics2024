...
<p align="center">
    <a href="{{ site.carpentries_site }}"><img src="{{ relative_root_path }}/assets/img/taxonomy_local_neighborhood.png" alt="Local neighborhood of a contig" width="500" /></a>
</p>
...
>     colors = [('seagreen' if names[n] == contig_id else 'lightblue') if names[n].startswith('contig') else colors[n] for n in subgraph.nodes()]
>
...
>     plt.legend(handles=patches, loc='center right', framealpha=0.5, frameon=True, prop={'size': 5})
>
>     ax = plt.gca()
>     ax.axis('off')
>     plt.tight_layout()
