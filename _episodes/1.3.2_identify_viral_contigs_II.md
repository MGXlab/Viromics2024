...
Use Jaeger's prediction scores and CheckV's estimates of completeness and contamination to separate phage from non-phage contigs. If you are an experienced programmer, write (a) script(s) for that. If you are not, use the solutions below. For the solutions below, we used file ```assembly_default_jaeger.tsv```, however, you could also use ```assembly_default_phages_jaeger.tsv```. The results of CheckV are located in the file `quality_summary.tsv` located in CheckV's output folder.
...
>     jaeger_selection = {row['contig_id'] for index, row in jaeger_df.iterrows() if row['reliability_score'] > 0.7}
>     checkv_selection = {row['contig_id'] for index, row in checkv_df.iterrows() if row['completeness'] > 50 and row['contamination'] < 5}
> 	
> 	# use set operation union to get the contigs in the jaeger set OR in the checkv set
>     joint_selection = jaeger_selection.union(checkv_selection)
> 	
>     # print some numbers
...
> #SBATCH --output=40_results_filter_contigs/filter_contigs.slurm.%j.out
> #SBATCH --error=40_results_filter_contigs/filter_contigs.slurm.%j.err
> 
...
> # for readability
> assembly='../1.2_virome_assembly/10_results_assembly_flye/cross_assembly/assembly.fasta'
> jaegerresults='10_jaeger/results_jaeger/assembly/assembly_default_jaeger.tsv'
> checkvresults='20_results_assessment_checkv/cross_assembly/quality_summary.tsv'
> 
> # run our script for filtering contigs based on the output of jaeger
> # and CheckV as well as the output path. 
> python 40_filter_contigs.py $assembly $jaegerresults $checkvresults 40_results_filter_contigs/assembly.fasta
> 
...
>
> 9. At your selected cutoffs, how many contigs get chosen by jaeger, how many by CheckV?
>
> 10. How large is the overlap between the two selections?
{: .challenge}
